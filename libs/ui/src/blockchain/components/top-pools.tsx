...
import CurrencySwapIcons from '../../components/currency-swap-icons';
import { CoinList } from '../../components/currency-swap-icons';
...
export default function TopPools({ limit }: TopPoolsProps) {
  ...
  return (
    ...
    >
      <h3 className="mb-6 text-base font-medium uppercase">Top agroSwap</h3>
      ...
      {TopPoolsData.slice(0, limit ?? -1).map((pool, index) => {
        const from = pool.from as CoinList;
        const to = pool.to as CoinList;
        return (
          <div
