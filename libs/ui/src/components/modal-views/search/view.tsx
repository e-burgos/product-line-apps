...
export function SearchTag({ label, link, search }: TagProps) {
  const { closeModal } = useModalViewStore();
  return (
    <AnchorLink
      to={link}
      onClick={() => {
        closeModal({ tag: label, search: search });
      }}
...
export function SearchFrom({ placeholder = 'Search...' }: SearchFromProps) {
  const { closeModal, setData } = useModalViewStore();
  ...
  return (
    <div className="relative" ref={ref} onFocus={() => setShowSuggestion(true)}>
      ...
      >
        ...
        <div className="flex h-12 w-12 shrink-0 cursor-pointer items-center justify-center rounded-full bg-white ltr:ml-3.5 rtl:ml-3.5 dark:bg-light-dark xl:hidden">
