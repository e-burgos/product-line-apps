...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    gl.clearColor(0, 0, 0, 0); // Asegurarse que el clearColor sea transparente en cada frame
...
const defaultItems: MenuItem[] = [
  {
    ...
    description: 'Destacaca con un look moderno y elegante.',
  },
];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      // @ts-ignore
      if (sketch?.gl) {
        // Siempre configurar el fondo transparente para que se vea la imagen detrás
        // @ts-ignore
        sketch?.gl?.clearColor(0, 0, 0, 0);
      }
    }
  ...
  // Actualizar configuración de WebGL cuando cambia el tema
  useEffect(() => {
    const canvas = canvasRef.current;
    if (canvas) {
  ...
  return (
    <div className="flex justify-center items-center w-[100vw] h-[calc(100vh-60px)] overflow-hidden relative bg-light dark:bg-dark">
      {/* Capa 1: Fondo difuminado con la imagen actual */}
      <div
        className="absolute inset-0"
        style={{
          backgroundImage: `url(${activeItem?.image || '/images/sol.jpg'})`,
      ...
      {/* Capa 2: Canvas con fondo transparente */}
      <div
      ...
      {activeItem && (
        <div className="absolute inset-0 z-20 pointer-events-none ">
          ...
          <div
            className={`
              ...
              ${isMoving && 'opacity-5 duration-[500ms] translate-y-[-200%]'}
            `}
          >
            <h2
              className={
                '[font-size:2rem] md:[font-size:3rem] lg:[font-size:3.5rem] text-left'
              }
            >
              {activeItem.title}
            </h2>
        ...
        `}
          >
            <p
              className={
                '[font-size:1.2rem] md:[font-size:1.5rem] lg:[font-size:2rem] text-right'
              }
            >
          ...
          ${
            isMoving
              ...
              : 'bottom-[3.8em] opacity-100 duration-[500ms] scale-100 -translate-x-1/2'
          }
        `}
          >
